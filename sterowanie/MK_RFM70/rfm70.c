...
void SPI_Read_Buf(uint8_t reg, uint8_t *pBuf, uint8_t len) {
	...
	SPI_PORT &= ~CSN;		// CSN = 0;
	pBuf[0] = SPI_RW(reg);   // Select register to write, and read status uint8_t
...
void RFM70_Init() {
	...
	RF_IRQDDR 	&= ~IRQ; // pin PB2 jako wejœcie
	SPI_PORT 	|= IRQ; // podci¹gniêcie do VDD
	GICR 		|= (1<<INT2); // zezwolenie na przerwanie zewnêtrzne
	MCUCSR 		&= (1<<ISC2);	// aktywacja zboczem opadaj¹cym
